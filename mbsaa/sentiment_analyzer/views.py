...
class FeatureExtractor:
    ...
    def normalize_emoticons(self, emoticon):
        """
        Normalizes an emoticon string
        """
        return "".join(OrderedDict.fromkeys(emoticon))

    def mask_urls(self, string):
        """
        Masks all urls in a string with a common symbol
        """
        return re.sub(http_re, '_url_', string)

    def get_word_feats(self, words):
        """
    ...
    def read_train_corpus(self, csv_corpus_file, tweet_col, sentiment_col):
        """
        Reads in a csv formatted train corpus file with the tweet text
        and sentiment in tweet_col and sentiment_col columns
        """
        train = []
        with open('full-corpus.csv', 'rb') as csv_file:
